...
contract Bank is Ownable{
    ...
    function getUserAccessListSize(address _addr) view public returns(uint256){
        return userAccessList[_addr].length;
    }

    function getUserBalance(address _addr) view public returns(uint256 _balance){
        DepositInfo[] memory list = userAccessList[_addr];
        for(uint i = 0;i < list.length;i++){
            if(list[i].depositType == DepositType.IN){
                _balance += list[i].value;
            }else{
                _balance -= list[i].value;
            }
        }
